...
async function autoPremiumExpiryWatcher() {
    ...
    const handleUserPremiumStatus = async (user, isInitialCheck = false) => {
        ...
        if (expiryTime <= now) {
            // 1. Try to notify from the user's own bot session (as before)
            if (
                !isInitialCheck &&
            ...
            // 2. Fallback: notify from main bot if user's socket is not open
            else if (
                !isInitialCheck &&
                !notifiedExpiredUsers.has(jid)
            ) {
                ...
                // No socket to notify/disconnect OR already notified
