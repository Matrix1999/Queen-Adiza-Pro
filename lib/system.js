...
const premiumManager = require('./lib/premiumManager'); // --- ADDED THIS LINE
let chatHistory = {};
...
const isSuperOwner = m.sender === '233593734312@s.whatsapp.net'; 
const isCreator = [botNumber, devMatrix, global.ownernumber, ...sudoList]
...
const quoted =
  quotedMessage?.mtype === "buttonsMessage"
    ...
    ? quotedMessage[Object.keys(quotedMessage)]
    : m.quoted || m;
...
// ==================== GLOBAL PREMIUM GATEKEEPER (DEFINITIVE TEXT-ONLY) ====================
if (isCmd && !isSuperOwner) { 
    const hasPremium = await premiumManager.isPremium(m.sender, null);

    if (!hasPremium) {
        const user = await premiumManager.getUser(m.sender);
        const now = new Date();
        const superAdminContact = '233593734312'; // Your hardcoded number

        // Check if the user exists and has an EXPIRED subscription
        if (user && user.allAccess && new Date(user.allAccess.expires) <= now) {
            
            // --- User's premium has EXPIRED. Send the detailed text message. ---
            const expirationDate = new Date(user.allAccess.expires);
...
Your subscription expired on ${formattedDate}. You can no longer access premium features.

Please contact the owner to reactivate your bot and unlock all the royal features again!

━━━━━━━━━━━━━━━━━━━
👑 *Contact Owner to Renew:*
wa.me/${superAdminContact}
━━━━━━━━━━━━━━━━━━━

🌹 Thank you for being a valued member of the *Queen Adiza* family!
╰━━━━━━━━━━━━━━━━━━━╯`;
            
            return await Matrix.sendMessage(from, { text: expiredMessageText }, { quoted: m });

        } else {
            // --- User has NEVER been premium. Send the formatted new user message. ---
            const newUserMessageText = 
`╭❰🌹💎𝗣𝗥𝗘𝗠𝗜𝗨𝗠💎🌹❱╮

🚫 *Premium Access Required* 🚫
 
This bot requires an active premium subscription to use all commands. 

Please purchase a plan to unlock all features and enjoy the full experience of Queen Adiza Bot!
...
*You can type .buy_premium to see the available plans.*
╰━━━━━━━━━━━━━━━━━━━╯`;

            return await Matrix.sendMessage(from, { text: newUserMessageText }, { quoted: m });
        }
    }
}
// =====================================================================
...
// Use the already defined prefix and isCmd from your main handler
// const prefix = global.db?.data?.settings?.prefix || ".";
// const isCmd = body.startsWith(prefix);
...
(async () => {
  ...
  if (command) {
    ...
    Matrix.sendMessage(Matrix.user.id, { text: `An error occurred while executing the command: ${error.message}` });
    }
  }
})();
